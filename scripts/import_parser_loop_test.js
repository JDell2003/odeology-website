...
function isImportWeightNoteLine(rawLine) {
  ...
  const hasLiftHint = /\b(last|last week|previous|prev|prior|working weight|top set|used|weight)\b/.test(lower);
  if (!hasLiftHint) return false;
  const hasLooseWeight = /\b(?:last|last week|previous|prev|prior|working weight|top set|used|weight)\b[^0-9]{0,12}\d{1,4}(?:\.\d+)?\s*s?\b/i.test(line);
  if (!hasWeight && !hasLooseWeight) return false;
  if (/\b(sets?|reps?|rest|x\s*\d|day|session)\b/.test(lower)) return false;
  return true;
}

function isImportLayoutLabelLine(rawLine) {
  const token = normalizeImportToken(rawLine);
  if (!token) return false;
  return /^(workout log|log|training log|session log|workout)$/.test(token);
}
...
function isLikelyExerciseName(raw) {
  ...
  if (/^\s*x\s*\d{1,2}(?:\s*[-\u2013\u2014]\s*\d{1,2})?\s*$/i.test(text)) return false;
  if (/^\s*\d{1,2}\s*[xX]\s*\d{1,2}(?:\s*[-\u2013\u2014]\s*\d{1,2})?\s*$/i.test(text)) return false;
  if (/^\s*\d{1,2}(?:\s*[-\u2013\u2014]\s*\d{1,2})?\s*(?:reps?)?\s*$/i.test(text)) return false;
  const lowerText = text.toLowerCase();
  if (/^\s*(last|last week|previous|prev|prior|working weight|top set|used|weight)\b/.test(lowerText)) return false;
  ...
  if (/^(?:x?\d{1,2}(?:-\d{1,2})?|\d{1,2}x\d{1,2}(?:-\d{1,2})?)$/.test(compact)) return false;
...
function parseImportedEntries(raw) {
  ...
  lines.forEach((line) => {
    if (isImportLayoutLabelLine(line)) return;
    ...
    const quickName = cleanImportedEntry(line);
    const looksLikeExerciseName = isLikelyExerciseName(quickName);
    const prescriptionHint = parseImportPrescriptionHint(line);
    if (prescriptionHint && chunks.length && !looksLikeExerciseName) {
