...
const DEFAULT_CTA_URL = String(process.env.KLAVIYO_FLOW_DEFAULT_CTA_URL || 'https://RiseForIt.up.railway.app/').trim();
...
async function getSenderDefaults() {
  try {
    ...
    const fromLabel = String(ci.default_sender_name || process.env.KLAVIYO_FROM_LABEL || 'RiseForIt').trim().slice(0, 120) || 'RiseForIt';
    if (!fromEmail) {
    ...
    return {
      fromEmail: envEmail,
      fromLabel: String(process.env.KLAVIYO_FROM_LABEL || 'RiseForIt').trim().slice(0, 120) || 'RiseForIt'
    };
  }
}
...
function buildUniversalTemplateHtml() {
  return [
    ...
    '          <tr><td style="padding:20px 24px;background:#111827;color:#f9fafb;font-size:22px;font-weight:700;letter-spacing:0.08em;">RiseForIt</td></tr>',
    '          <tr>',
    '            <td style="padding:24px;">',
    '              <div style="font-size:21px;line-height:1.3;font-weight:700;margin:0 0 12px 0;">{{ event.ode_email_subject|default:"RiseForIt update" }}</div>',
    '              <div style="font-size:15px;line-height:1.45;color:#4b5563;margin:0 0 18px 0;">{{ event.ode_email_preheader|default:"You have a new update in RiseForIt." }}</div>',
    '              <div style="font-size:15px;line-height:1.55;white-space:pre-line;color:#111827;margin:0 0 20px 0;">{{ event.ode_email_text|default:"Open RiseForIt to view your latest update." }}</div>',
    '              <table role="presentation" cellspacing="0" cellpadding="0" border="0">',
    '                <tr>',
    '                  <td style="background:#d38b2c;border-radius:8px;">',
    `                    <a href="{{ event.ode_email_cta_url|default:"${DEFAULT_CTA_URL}" }}" style="display:inline-block;padding:12px 18px;color:#111827;font-size:15px;font-weight:700;text-decoration:none;">{{ event.ode_email_cta_label|default:"Open RiseForIt" }}</a>`,
    '                  </td>',
    '                </tr>',
    '              </table>',
    ...
    actions: [
      {
        ...
        data: {
          message: {
            ...
            cc_email: null,
            bcc_email: null,
            subject_line: '{{ event.ode_email_subject|default:"RiseForIt update" }}',
            preview_text: '{{ event.ode_email_preheader|default:"You have a new RiseForIt update." }}',
